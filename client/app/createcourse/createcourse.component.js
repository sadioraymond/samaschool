...
export function ModalInstanceCtrl($uibModalInstance, items, userProvider, classeProvider, Auth, coursProvider, $state, $log, $timeout, $stateParams, multerProvider) {
    ...
    $ctrl.ok = function() {
        ...
        //pour fermer le modal apré selection de classe
        $uibModalInstance.close($ctrl.selected.item);
        ...
        $log.log('images bi', $ctrl.parametre);
        if ($stateParams.id !== "") {
            coursProvider.objetCours._id = $stateParams.id
        } else {
            //ajout du cours à travers le provider en passant l'objet complet du cours et le nom de l'image du cours
            coursProvider.ajoutCours(coursProvider.objetCours, $ctrl.parametre);
        }
    };
