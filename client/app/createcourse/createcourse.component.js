...
export class CreatecourseComponent {
  ...
  getClasseByUser(user) {
    this.classeProvider.getClasseByUser(user).then(list => {
        ...
        for (let i = 0; i < this.listClasseUser.length; i++) {
          for (let j = 0; j < this.listClasseUser[i].length; j++) {
            if (this.verif(this.listClasseUsers[k], this.listClasseUser[i][j].etablissement._id)) {
              userClass[`${l}`] = this.listClasseUser[i][j].classe;
              l++;
            }
          }
        }
...
export function ModalInstanceCtrl($uibModalInstance, items, userProvider, classeProvider, Auth) {
  var $ctrl = this;
  $ctrl.items = items;
  $ctrl.selected = {
    item: $ctrl.items[0]
  ...
  $ctrl.ok = function () {
    ...
    classeProvider.selectedClasses = $ctrl.selectedClass;
  };
...
ModalInstanceCtrl.$inject = ["$uibModalInstance", "items", "userProvider", "classeProvider", "Auth"];
