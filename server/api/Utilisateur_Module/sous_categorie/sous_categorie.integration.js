...
describe('SousCategorie API:', function() {
  ...
  describe('GET /api/sous_categories/:id', function() {
    ...
    beforeEach(function(done) {
      request(app)
        .get(`/api/sous_categories/${newSousCategorie._id}`)
        .expect(200)
        .expect('Content-Type', /json/)
        .end((err, res) => {
          if(err) {
  ...
  describe('PUT /api/sous_categories/:id', function() {
    ...
    beforeEach(function(done) {
      request(app)
        .put(`/api/sous_categories/${newSousCategorie._id}`)
        .send({
  ...
  describe('PATCH /api/sous_categories/:id', function() {
    ...
    beforeEach(function(done) {
      request(app)
        .patch(`/api/sous_categories/${newSousCategorie._id}`)
        .send([
  ...
  describe('DELETE /api/sous_categories/:id', function() {
    it('should respond with 204 on successful removal', function(done) {
      request(app)
        .delete(`/api/sous_categories/${newSousCategorie._id}`)
        .expect(204)
        .end(err => {
          if(err) {
            return done(err);
